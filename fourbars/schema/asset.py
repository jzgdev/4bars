...
class Asset(object):
    ...
    def process_original(self, in_asset_type):
        ...
        ai.append_md5(self.md5(ai.org_abs_path))
    ...
    def process_aac(self, in_asset_type):
        ...
        out, _ = (ffmpeg
                  .input(ai.org_abs_path)
                  .output(ai.lib_abs_full_file, progress='-', acodec='aac', **{'b:a': '{0}'.format(in_asset_type.value.split('_')[1])})
                  .overwrite_output()
                  .run()
                  )
        ai.append_probe(ffmpeg.probe(ai.lib_abs_full_file, cmd='ffprobe'))
...
#    nb_frames = None
    ...
    def append_probe(self, in_probe):
        ...
        # self.bits_per_raw_sample = in_probe['streams'][0]['bits_per_raw_sample']
        # self.nb_frames = in_probe['streams'][0]['nb_frames']
